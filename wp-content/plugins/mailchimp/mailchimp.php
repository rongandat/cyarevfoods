    ...
    if (is_array($lists) && !empty($lists) && isset($_POST['mc_list_id'])) {
        ...
        if ($list_id != '') {
            ...
            $msg = '<p class="success_msg">' .
                    sprintf(
                            __('Success! Loaded and saved the info for %d Merge Variables', 'mailchimp_i18n') . $igs_text, count($mv)
                    ) . ' ' .
...
// If we don't have an API Key, do a login form
        ...
        <?php esc_html_e('To start using the MailChimp plugin, we first need to login and get your API Key. Please enter your MailChimp API Key below.', 'mailchimp_i18n'); ?>
                    ...
                    <?php
                    echo sprintf(
                            '%1$s <a href="http://www.mailchimp.com/signup/" target="_blank">%2$s</a>', esc_html(__("Don't have a MailChimp account?", 'mailchimp_i18n')), esc_html(__('Try one for Free!', 'mailchimp_i18n'))
                    );
                    ?>
...
//Just get out if nothing else matters...
        ...
        if (count($lists) == 0) {
            ?>
                        ...
                        <?php
                        echo sprintf(
                                esc_html(__("Uh-oh, you don't have any lists defined! Please visit %s, login, and setup a list before using this tool!", 'mailchimp_i18n')), "<a href='http://www.mailchimp.com/'>MailChimp</a>"
                        );
                        ?>
                        </span>
                            <?php
    ...
    <?php wp_nonce_field('update_general_form_settings', '_mcsf_nonce_action'); ?>
                ...
                <table class="widefat">
                    <tr valign="top">
                        ...
                        <td><input name="mc_rewards" type="checkbox"<?php if (get_option('mc_rewards') == 'on' || get_option('mc_rewards') == '') {
