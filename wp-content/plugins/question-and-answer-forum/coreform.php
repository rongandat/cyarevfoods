...
if (!is_user_logged_in()) {
    if (get_option('q_loginrequired_ask') == "TRUE") {    //force users to log in
        ...
        return $html;
    } elseif (get_option('q_loginrequired_ask') == "name-and-email") {         //textboxes for name and email
        $userinfo_html .= <<<HTML
...
$html .= <<<HTML
<input type="text" class="st-forminput" style="width:300px" value="$qtitle" id="question-title-box-$questionformid" name="title" onfocus="if(this.value == '$t1'){this.value = '';}" onblur="if(this.value == ''){this.value = '$t1';}" />
HTML;
...
$html .= <<<HTML
<textarea class="st-forminput" style="width: 508px;height: 165px;" cols="80" rows="5" id="question-box-$questionformid" name="question" onfocus="if(this.value == '$t2'){this.value = '';}" onblur="if(this.value == ''){this.value = '$t2';}">$Qtext</textarea>
HTML;
...
$html .= '<a class="ask" href="javascript:jQuery(\'#questionform-'.$questionformid.'\').submit()">' . __('Ask', 'qna-forum') . '</a>'
?>