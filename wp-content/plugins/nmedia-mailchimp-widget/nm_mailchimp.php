...
class nmMailChimp extends WP_Widget {
    ...
    function nmMailChimp() {
        parent::WP_Widget('nmedia_mail_chimp', 'Revfoods MailChimp', array('description' => 'MailChimp Widget by revfoods'));
    }
    ...
    public function set_up_admin_page() {
        ...
        add_submenu_page('options-general.php', 'MailChimp Widget Options', 'MailChimp Widget', 'activate_plugins', __FILE__, array('nmMailChimp', 'admin_page'));
    }
    ...
    function widget($args, $instance) {
        ...
        /* print_r($args); */
        nmMailChimp::nm_load_form(
                $instance['nm_mc_form_id'], 
                $instance['nm_mc_title'], 
                $instance['headline_font_size'], 
                $instance['headline_font_color'], 
    ...
    function form($instance) {
        $default = array('nm_mc_title' => __('join the revolution'),
            'headline_font_size' => '86px',
            'headline_font_color' => '#59ba47',
            'nm_mc_detail' => 'Lorem ipsum dolor sit amet consultiteur. Erat sit amet lorem dolor. 
    ...
    function nm_load_form($fid, $boxTitle, $headline_font_size, $headline_font_color, $detail, $buttonText, $widget_id) {
    ...
    function saveForm($formName, $formDetail, $lid, $groups, $vars) {
        ...
        $wpdb->insert($wpdb->prefix . nmMailChimp::$tblName, $dt
        );
    ...
    function fixRequestURI($vars) {
        ...
        if ($qsStr != "")
            $qsStr = substr($qsStr, 0, strlen($qsStr) - 1);
        $uri = $parts[0];
        if ($qsStr != "")
            $uri.="?" . $qsStr;
        return $uri;
